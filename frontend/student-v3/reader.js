...
function feedbackBody(q,feedback) {
  if(state.tab==="explanation") {
    const evidence=(feedback.evidence||[]), detail=feedback.structured_explanation||{}, keyPoints=detail.key_points||feedback.points||[], reasoning=detail.clinical_reasoning||[];
    const reasoningHtml=(reasoning.length?reasoning:[detail.summary||feedback.explanation||"등록된 해설이 없습니다."]).map((paragraph)=>`<p>${esc(paragraph)}</p>`).join("");
    return `<div class="explanation-hero"><span>핵심 결론</span><strong>${esc(detail.conclusion||detail.summary||feedback.explanation||"등록된 해설이 없습니다.")}</strong>${detail.correct_answer?`<small>정답 · ${esc(detail.correct_answer)}</small>`:""}</div>
      ...
      ${detail.axis_focus?.message?`<div class="axis-focus"><span>10-Axis</span><strong>${esc(detail.axis_focus.message)}</strong></div>`:""}
      ...
      <h3 class="feedback-subtitle">선지별 분석</h3><div class="choice-explanations">${q.choices.map((choice)=>`<div class="choice-expl"><b>${esc(choice.n)}</b><span>${esc(feedback.choice_explanations?.[String(choice.n)]||"선택지별 해설이 없습니다.")}</span></div>`).join("")}</div>
